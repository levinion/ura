...
M.register("tiling", {
	...
	apply = function(win)
		...
		assert(usable)
		local workspace = ura.api.get_current_workspace()
		assert(workspace)

		local workspaces = ura.api.get_indexed_workspaces(output)
		assert(workspaces)

		local opt = ura.api.get_option("layout.tiling")
...
ura.hook.set("window-new", function(e)
	...
	local workspaces = ura.api.get_indexed_workspaces(output)
	assert(workspaces)
	for index = 0, #workspaces - 1 do
		local w = ura.api.get_window(ws, index)
		if w and w ~= win then
			M.apply(w)
		end
	end
end, { ns = "layout.tiling", priority = 40 })
