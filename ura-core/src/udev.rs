...
use crate::{controller::Controller, state::SurfaceDmabufFeedback};
use crate::{
    drawing::*,
    render::*,
    shell::WindowElement,
    state::{post_repaint, take_presentation_feedback, Backend, CalloopData, State},
};
...
pub fn run_udev(controller: Box<dyn Controller>) {
    ...
    let data = UdevData {
        ...
        pointer_image: crate::cursor::Cursor::load(&controller),
    ...
    let mut state = State::init(display, event_loop.handle(), data, controller, true);
        ...
        .insert_source(notifier, move |event, &mut (), data| match event {
            ...
            SessionEvent::ActivateSession => {
                    ...
                    .map(|(handle, backend)| (*handle, backend))
                {
                    ...
                    if let Some(lease_global) = backend.leasing_global.as_mut() {
                        lease_global.resume::<State<UdevData>>();
                    }
...
impl DrmLeaseHandler for State<UdevData> {
...
delegate_drm_lease!(State<UdevData>);
...
impl Drop for SurfaceData {
    fn drop(&mut self) {
        if let Some(global) = self.global.take() {
            self.dh.remove_global::<State<UdevData>>(global);
        }
    }
}
    ...
    fn device_added(&mut self, node: DrmNode, path: &Path) -> Result<(), DeviceAddError> {
        ...
        self.backend_data.backends.insert(
            ...
            BackendData {
                ...
                leasing_global: DrmLeaseState::new::<State<UdevData>>(&self.display_handle, &node)
                    .map_err(|err| {
    ...
    fn connector_connected(
        ...
        if non_desktop {
            ...
            if let Some(lease_state) = device.leasing_global.as_mut() {
                lease_state.add_connector::<State<UdevData>>(
                    connector.handle(),
                    output_name,
                    format!("{} {}", make, model),
                );
    ...
    fn device_removed(&mut self, node: DrmNode) {
        ...
        if let Some(mut backend_data) = self.backend_data.backends.remove(&node) {
            if let Some(mut leasing_global) = backend_data.leasing_global.take() {
                leasing_global.disable_global::<State<UdevData>>();
            }
