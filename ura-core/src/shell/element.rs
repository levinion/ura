...
impl<Backend: crate::state::Backend> PointerTarget<State<Backend>> for SSD {
    fn enter(&self, _seat: &Seat<State<Backend>>, _data: &mut State<Backend>, event: &MotionEvent) {
    ...
    fn motion(
        &self,
        _seat: &Seat<State<Backend>>,
        _data: &mut State<Backend>,
        event: &MotionEvent,
    ) {
        let mut state = self.0.decoration_state();
        if state.is_ssd {
    ...
    fn button(&self, seat: &Seat<State<Backend>>, data: &mut State<Backend>, event: &ButtonEvent) {
        let mut state = self.0.decoration_state();
        if state.is_ssd {
            state.header_bar.clicked(seat, data, &self.0, event.serial);
        }
    }
    fn axis(&self, _seat: &Seat<State<Backend>>, _data: &mut State<Backend>, _frame: AxisFrame) {}
    fn frame(&self, _seat: &Seat<State<Backend>>, _data: &mut State<Backend>) {}
    fn leave(
        &self,
        _seat: &Seat<State<Backend>>,
        _data: &mut State<Backend>,
...
impl<Backend: crate::state::Backend> TouchTarget<State<Backend>> for SSD {
    fn down(
        &self,
        seat: &Seat<State<Backend>>,
        data: &mut State<Backend>,
    ...
    fn frame(&self, _seat: &Seat<State<Backend>>, _data: &mut State<Backend>, _seq: Serial) {}

    fn cancel(&self, _seat: &Seat<State<Backend>>, _data: &mut State<Backend>, _seq: Serial) {}
