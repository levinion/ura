...
impl<BackendData: Backend> ServerDndGrabHandler for State<BackendData> {
    fn send(&mut self, _mime_type: String, _fd: OwnedFd, _seat: Seat<Self>) {
        unreachable!("Ura doesn't do server-side grabs");
    }
}
...
delegate_tablet_manager!(@<BackendData: Backend + 'static> State<BackendData>);

delegate_text_input_manager!(@<BackendData: Backend + 'static> State<BackendData>);
...
delegate_input_method_manager!(@<BackendData: Backend + 'static> State<BackendData>);

impl<BackendData: Backend> KeyboardShortcutsInhibitHandler for State<BackendData> {
...
delegate_keyboard_shortcuts_inhibit!(@<BackendData: Backend + 'static> State<BackendData>);

delegate_virtual_keyboard_manager!(@<BackendData: Backend + 'static> State<BackendData>);
...
delegate_pointer_constraints!(@<BackendData: Backend + 'static> State<BackendData>);
        ...
        with_states(&surface, |states| {
            ...
            if let Some(output) = primary_scanout_output {
                ...
                output.change_current_state(
                    ...
                    None,
                    Some(Scale::Fractional(self.controller.scale_factor())),
                    None,
                );
...
impl<BackendData: Backend + 'static> SecurityContextHandler for State<BackendData> {
    fn context_created(
        &mut self,
...
delegate_security_context!(@<BackendData: Backend + 'static> State<BackendData>);

#[cfg(feature = "xwayland")]
impl<BackendData: Backend + 'static> XWaylandKeyboardGrabHandler for State<BackendData> {
...
#[cfg(feature = "xwayland")]
delegate_xwayland_keyboard_grab!(@<BackendData: Backend + 'static> State<BackendData>);

impl<BackendData: Backend> XdgForeignHandler for State<BackendData> {
    fn xdg_foreign_state(&mut self) -> &mut XdgForeignState {
        &mut self.xdg_foreign_state
    }
}
smithay::delegate_xdg_foreign!(@<BackendData: Backend + 'static> State<BackendData>);
        ...
        let xwayland = {
            ...
            let ret = handle.insert_source(channel, move |event, _, data| match event {
                XWaylandEvent::Ready {
                    ...
                    let cursor = Cursor::load(&data.state.controller);
