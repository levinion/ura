...
pub fn run_winit(controller: Box<dyn Controller>) {
    ...
    let _global = output.create_global::<State<WinitData>>(&display.handle());
    ...
    let dmabuf_state = if let Some(default_feedback) = dmabuf_default_feedback {
        ...
        let dmabuf_global = dmabuf_state.create_global_with_default_feedback::<State<WinitData>>(
            &display.handle(),
            &default_feedback,
        );
        ...
        let mut dmabuf_state = DmabufState::new();
        let dmabuf_global =
            dmabuf_state.create_global::<State<WinitData>>(&display.handle(), dmabuf_formats);
