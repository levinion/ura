    ...
    fn process_common_key_action(&mut self, action: KeyAction) {
        match action {
                ...
                for element in self.space.elements() {
                    ...
                    if let Some(toplevel) = element.0.toplevel() {
                        toplevel.with_pending_state(|state| {
                            if state.decoration_mode.is_none() {
                                let new_mode = zxdg_toplevel_decoration_v1::Mode::ServerSide;
            ...
            KeyAction::ToggleFullScreen => {
                // let surface = self
                //     .surface_under(self.pointer.current_location())
                //     .unwrap()
                //     .0;
                // self.fullscreen_request(surface.wl_surface().unwrap(), None);
            }
                ...
                |data, modifiers, handle| {
                    ...
                    if let KeyState::Pressed = state {
                        if !inhibited {
                            ...
                            let action = controller.process_keyboard_shortcut(*modifiers, keysym);
...
#[cfg(any(feature = "winit", feature = "x11"))]
impl<Backend: crate::state::Backend> State<Backend> {
